...
### Data import
All input data have been created with QIIME2 and are imported in R (v. 4.2.0). QIIME2 scripts and parameter settings are found in separate bash files that can be found in the dedicated [Github repository](https://github.com/pietervanveelen/Larasati_Mn_oxidation_16SrRNA).<br>
With this RMarkdown document, the repository contains input data (metadata and QIIME2 .qza files), QIIME2 and R scripts in the scripts folder for full reproducibility of the reported results and figures.
...
```{r rel abund barplot, message=F, echo=T, eval=T, warning=T, include=T, cache=T, fig.width=5, fig.height=8}

#input data
# psdata_OBER_rare # rarefied ASV level
# Genus agglomerated after rarefied at ASV level
psdata_OBER_rare_Genus 

ps_Genus<- psmelt(transform_sample_counts(psdata_OBER_rare_Genus, fun = function(x) x/sum(x)))

### write rel abund data plus other tax info (min1pct)
ps_Genus %>% 
  # filter(!grepl("mock", combined), # remove mock communities
  #        Sample_mock != "blank") %>% # remove NC blanks
  as_tibble() %>% 
  ...
  ungroup() %>% 
  mutate(across(where(is.numeric), ~round(.x, digits = 2))) %>% 
  write_csv("output_data/OBER_proj3_MnOx_relative_Genus_abundances_sorted_by_overall_mean.csv", col_names = T, append = F)


### Genus filtered > min 1 pct 
psdata_OBER_rare_Genus_min1pct # filtered to retain >1%

ps_Genus_min1pct <- psmelt(transform_sample_counts(psdata_OBER_rare_Genus_min1pct, fun = function(x) x/sum(x)))
...
Genus_abundances_1pct <- ps_Genus_min1pct %>% 
  as_tibble() %>% 
  select(Sample, Sample_name, Sample_type, Replicate, Family, Genus, Abundance) %>% 
  group_by(Sample, Sample_name, Sample_type, Replicate, Family, Genus) %>% 
  summarize(rel_abund = sum(Abundance), .groups = "drop") %>% 
...
Genus_pool_1pct <- Genus_abundances_1pct %>% 
  group_by(Genus) %>% 
  summarize(pool = max(mean_rel_abund) < 1, 
            mean = mean(mean_rel_abund), 
            .groups = "drop")
  
inner_join(Genus_abundances_1pct, Genus_pool_1pct, by="Genus") %>% 
